...
namespace Eagle
{
	void ScreenPass::initialize(ScreenPassInitInfo init_info)
	{
		VulkanPass::initialize({ init_info.rhi , nullptr });
		m_resolution = init_info.out_resolution;
		m_framebuffer.width = m_resolution.x;
		m_framebuffer.height = m_resolution.y;
		m_src_texture_ptrs.clear();
		m_vert_path = init_info.vert_path;
		m_frag_path = init_info.frag_path;
	}
	...
	void ScreenPass::cleanupSwapChain()
	{
		VulkanPass::cleanupSwapChain();
	}

	void ScreenPass::cleanup()
	{
		VulkanPass::cleanup();
	}

	void ScreenPass::setupAttachments()
	{
	...
	void ScreenPass::setupRenderPass()
	{
		std::vector<VkAttachmentDescription> attachment(m_framebuffer.attachments.size());
		std::vector<VkAttachmentReference> colorAttachmentRef(m_framebuffer.attachments.size());

		for (int i = 0; i < attachment.size(); i++) {
			VkAttachmentDescription& attachment_description = attachment[i];
			attachment_description.format = m_framebuffer.attachments[i].format;
			...
			VkAttachmentReference& attachment_reference = colorAttachmentRef[i];
	...
	void ScreenPass::setupDescriptorSetLayout()
	{
		m_descriptors.resize(1);

		{
			std::vector<VkDescriptorSetLayoutBinding> layoutBinding(m_src_texture_ptrs.size());
			for (int i = 0; i < m_src_texture_ptrs.size(); i++) {
				layoutBinding[0].binding = i;
	...
	void ScreenPass::setupPipelines()
	{
		...
		auto vertShaderCode = FileSystem::readFile(m_vert_path);
		auto fragShaderCode = FileSystem::readFile(m_frag_path);
	...
	void ScreenPass::setupFramebuffers()
	{
		std::vector<VkImageView> attachments(m_framebuffer.attachments.size());
		for (int i = 0; i < m_framebuffer.attachments.size(); i++) {
			attachments[i] = m_framebuffer.attachments[i].view;
		}
	...
	void ScreenPass::setupUniformBuffers()
	{
	}

	void ScreenPass::setupDescriptorSets()
	{
		{
			...
			std::vector<VkDescriptorImageInfo> image_infos(m_src_texture_ptrs.size());
			std::vector<VkWriteDescriptorSet> write_infos(m_src_texture_ptrs.size());
			for (int i = 0; i < m_src_texture_ptrs.size(); i++) {
				image_infos[i].sampler = VulkanUtil::getNearestSampler(m_rhi->m_physical_device, m_rhi->m_device);
				image_infos[i].imageView = m_src_texture_ptrs[i]->view;
				image_infos[i].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

				write_infos[i].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
				write_infos[i].pNext = NULL;
				write_infos[i].dstSet = m_descriptors[0].descriptor_set;
				write_infos[i].dstBinding = i;
				write_infos[i].dstArrayElement = 0;
				write_infos[i].descriptorType = VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT;
				write_infos[i].descriptorCount = 1;
				write_infos[i].pImageInfo = &image_infos[i];
			}

			vkUpdateDescriptorSets(m_rhi->m_device, m_src_texture_ptrs.size(), write_infos.data(), 0, nullptr);
		}
	}

	void ScreenPass::updateRecreateSwapChain()
	{
		cleanupSwapChain();
	...
	void ScreenPass::addSourceTexture(VulkanAttachment* src_texture_ptr)
	{
		m_src_texture_ptrs.push_back(src_texture_ptr);
	}

	void ScreenPass::addOutputTarget(VkFormat out_format)
	{
		VulkanAttachment n_attachment{};
		n_attachment.format = out_format;
		m_framebuffer.attachments.push_back(n_attachment);
	}
}